...
object GmailAuth {
    ...
    fun checkTokenStatus(): TokenCheckResult {
        ...
        println("[GmailAuth] Token file found: ${PollerConfig.GMAIL_TOKEN_FILE}")

        try {
            val clientSecrets = GoogleClientSecrets.load(
                GsonFactory.getDefaultInstance(),
                InputStreamReader(FileInputStream(PollerConfig.GMAIL_CREDENTIALS_FILE))
            )
    ...
    fun getCredentials(): Credential {
        val tokenPath = Path.of(PollerConfig.GMAIL_TOKEN_FILE)
        if (Files.exists(tokenPath)) {
            println("[GmailAuth] Found stored token at: ${PollerConfig.GMAIL_TOKEN_FILE}")
            try {
                val tokenStream = Files.newInputStream(tokenPath)
                val response = GsonFactory.getDefaultInstance()
                    .fromInputStream(tokenStream, GoogleTokenResponse::class.java)
            ...
            println("[GmailAuth] No stored token found at: ${PollerConfig.GMAIL_TOKEN_FILE}")
        }
