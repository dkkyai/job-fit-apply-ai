...
class Notifier(
    ...
    /** Returns true if a message was sent (false = skipped: no channels, or a non-job event). */
    fun notify(event: CompletedEvent): Boolean {
        if (!client.discordConfigured && !client.telegramConfigured) return false
        ...
        if (event.company.isNullOrBlank() && event.error == null) return false

        val company = event.company ?: ""
        val title = (event.roleTitle ?: "").ifBlank { "*(no title)*" }
        if (event.error != null) {
            client.postDiscord("• $company — $title — error: ${event.error}")
            return true
        }
        val score = event.fitScore?.toString() ?: "?"
        val action = event.pipelineAction ?: "?"
        client.postDiscord("• ${discordJobLabel(event)} — **$score** ($action)")
        if ((event.fitScore ?: 0) >= fitThreshold) {
            client.postTelegramHtml("High-fit: ${telegramJobLabel(event)} — ${event.fitScore}")
        }
        return true
    }
